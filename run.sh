...
git pull && sudo docker build -f Dockerfile -t whisper-api . && sudo docker run -d --gpus all --network=host --privileged -v /jax-cache:/jax-cache -v /huggingface-cache:/root/.cache/huggingface/hub whisper-api
